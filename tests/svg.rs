...
#[test]
fn test_qr_v40_ec_h_as_svg() {
    let code = QrCode::with_version(b"01234567", Version::Normal(40), EcLevel::H).unwrap();
    let image = code.render::<Color<'_>>().build();
    let expected = include_str!("data/test_qr_v40_ec_h_as_svg.svg");
    assert_eq!(&image, expected);
}

#[test]
fn test_micro_qr_v4_ec_q_as_svg() {
    let code = QrCode::with_version(b"01234567", Version::Micro(4), EcLevel::Q).unwrap();
    let image = code.render::<Color<'_>>().build();
    let expected = include_str!("data/test_micro_qr_v4_ec_q_as_svg.svg");
    assert_eq!(&image, expected);
}

#[test]
fn test_rmqr_vr17x139_ec_h_as_svg() {
    let code = QrCode::with_version(b"01234567", Version::RectMicro(17, 139), EcLevel::H).unwrap();
    let image = code.render::<Color<'_>>().build();
    let expected = include_str!("data/test_rmqr_vr17x139_ec_h_as_svg.svg");
    assert_eq!(&image, expected);
}
